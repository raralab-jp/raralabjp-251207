...
def first_content_line(text: str) -> str:
    skip_labels = (
        ...
        "商品番号", "石種", "ファセットデザイン", "デザイナー", "研磨", "重さ", "サイズ",
        "産地", "処理", "透明度", "鑑別", "デザイン改変",
    )
...
product_id = pick(["Product ID", "ProductID", "ID", "商品番号"], text, default="")

stone = pick(["Stone"], text, default="").strip()
stone_ja = pick(["石種"], text, default="").strip()


design_name_raw = pick(["Facet Design", "Design", "ファセットデザイン"], text, default="")
designer = pick(["Designed by", "Designer", "デザイナー"], text, default="")

faceted_by = pick(["Faceted by"], text, default="").strip()
...
# -----------------------------
# Stone / Carat fallbacks
# -----------------------------
has_stone_label = bool(stone)
if has_stone_label and not slugify(stone):
    warn("Stone label exists but cannot be slugified. Use English in 'Stone:' if slug is needed.")

carat_labeled = parse_carat_value(pick(["Carat", "重さ"], text, default=""))
has_carat_label = bool(carat_labeled)
carat = carat_labeled if has_carat_label else ""
...
# -----------------------------
# Defaults allowed in master only where explicitly requested
# -----------------------------
if not cert_lab_ja:
    cert_lab_ja = "日独宝石研究所"
if not cert_lab_en:
    cert_lab_en = "Japan Germany Gemmological Laboratory"

# -----------------------------
# Carat validation / slug default
# -----------------------------
...
# NOTE:
# title_jp は完成文の自動生成値を保存しない。
# 明示入力があった場合のみ保持する。
title_jp_value = title_jp_input.strip()
...
ap.add_argument("--title_jp", dest="title_jp", default=title_jp_value)
...
# -----------------------------
# --csv オプション指定時：重複チェック付きでファイルに書き込む
# 指定なし：従来通り標準出力
# -----------------------------
csv_path = None
for i, a in enumerate(sys.argv[1:], 1):
    if a == "--csv" and i < len(sys.argv) - 1:
...
if csv_path:
    ...
    if not p.exists():
        ...
        print(f"[csv] created: {csv_path}", file=sys.stderr)
    elif not product_id:
        # product_id なし：重複チェックせず末尾に追記
        with p.open("a", encoding="utf-8", newline="") as f:
            f.write(buf.getvalue().splitlines()[1] + "\n")  # ヘッダー行を除いたデータ行のみ追記
        print(f"[csv] appended (no product_id): {csv_path}", file=sys.stderr)
    else:
        # product_id あり：既存行を読み込んで重複チェック
        existing_rows = []
        ...
        for i, r in enumerate(existing_rows):
            if r.get("product_id", "").strip() == product_id:
                existing_rows[i] = row
                replaced = True
                break
        ...
        print(f"[csv] {action} (product_id={product_id}): {csv_path}", file=sys.stderr)
else:
